...
export const useBoards = (token, onLogout, navigate) => {
  ...
  const sanitizeBoardData = useCallback((data, exclude = []) => {
    const allowed = {
      name: data.name,
  ...
  const fetchBoardsByGate = useCallback(async (gateId, filters = {}, signal) => {
  ...
  const fetchBoardsByClass = useCallback(async (classId, filters = {}, signal) => {
  ...
  const fetchBoard = useCallback(async (boardId, gateId, classId, signal) => {
  ...
  const createNewBoard = useCallback(async (boardData) => {
    ...
    setError(null);
    try {
      const sanitizedData = sanitizeBoardData(boardData);
      const newBoard = await createBoard(sanitizedData, token);
  ...
  }, [token, sanitizeBoardData, handleAuthError]);
  ...
  const createNewBoardInGate = useCallback(async (gateId, boardData) => {
    ...
    setError(null);
    try {
      const sanitizedData = sanitizeBoardData(boardData, ['gate_id', 'class_id']);
      const newBoard = await createBoardInGate(gateId, sanitizedData, token);
      setBoards((prev) => [...prev, newBoard]);
      return newBoard;
    } catch (err) {
      return handleAuthError(err);
  ...
  const createNewBoardInClass = useCallback(async (classId, boardData) => {
    ...
    try {
      const sanitizedData = sanitizeBoardData(boardData, ['gate_id', 'class_id']);
      const newBoard = await createBoardInClass(classId, sanitizedData, token);
      setBoards((prev) => [...prev, newBoard]);
      return newBoard;
    } catch (err) {
      return handleAuthError(err);
  ...
  const updateExistingBoard = useCallback(async (boardId, gateId, classId, boardData) => {
    if (!token || !boardId || !boardData) return setError("Authentication, board ID, or data missing.");
    setLoading(true);
    setError(null);
    try {
      const sanitizedData = sanitizeBoardData(boardData);
      const updatedBoard = await updateBoard(boardId, gateId, classId, sanitizedData, token);
  ...
  const deleteExistingBoard = useCallback(async (boardId, gateId, classId) => {
    if (!token || !boardId) return setError("Authentication or board ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const fetchBoardMembersList = useCallback(async (boardId) => {
    if (!token || !boardId) return setError("Authentication or board ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const likeExistingBoard = useCallback(async (boardId) => {
    if (!token || !boardId) return setError("Authentication or board ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const unlikeExistingBoard = useCallback(async (boardId) => {
    if (!token || !boardId) return setError("Authentication or board ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const fetchBoardStatistics = useCallback(async (boardId) => {
    if (!token || !boardId) return setError("Authentication or board ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const inviteUserToBoard = useCallback(async (boardId, anonymousId) => {
  ...
  const acceptBoardInvite = useCallback(async (boardId) => {
    if (!token || !boardId) return setError("Authentication or board ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const addBoardMember = useCallback(async (boardId, anonymousId, role = "viewer") => {
    if (!token || !boardId || !anonymousId) return setError("Authentication, board ID, or user ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const removeBoardMember = useCallback(async (boardId, anonymousId) => {
    if (!token || !boardId || !anonymousId) return setError("Authentication, board ID, or user ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const runBoardAIModeration = useCallback(async (boardId) => {
    if (!token || !boardId) return setError("Authentication or board ID missing.");
    setLoading(true);
    setError(null);
    try {
