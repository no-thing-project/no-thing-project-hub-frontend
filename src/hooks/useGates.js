...
export const useGates = (token, onLogout, navigate) => {
  ...
  const [stats, setStats] = useState(null); // Для аналітики
  ...
  const fetchGatesList = useCallback(async (filters = {}, signal) => {
  ...
  const createNewGate = useCallback(async (gateData) => {
    if (!token || !gateData) return setError("Authentication or gate data missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const updateExistingGate = useCallback(async (gateId, gateData) => {
    if (!token || !gateId || !gateData) return setError("Authentication, gate ID, or data missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const updateGateStatusById = useCallback(async (gateId, statusData) => {
    if (!token || !gateId || !statusData) return setError("Authentication, gate ID, or status data missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const addMemberToGate = useCallback(async (gateId, memberData) => {
    if (!token || !gateId || !memberData) return setError("Authentication, gate ID, or member data missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const removeMemberFromGate = useCallback(async (gateId, memberId) => {
    if (!token || !gateId || !memberId) return setError("Authentication, gate ID, or member ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const fetchGateMembersList = useCallback(async (gateId) => {
    if (!token || !gateId) return setError("Authentication or gate ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const likeGateById = useCallback(async (gateId) => {
    if (!token || !gateId) return setError("Authentication or gate ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  const unlikeGateById = useCallback(async (gateId) => {
    if (!token || !gateId) return setError("Authentication or gate ID missing.");
    setLoading(true);
    setError(null);
    try {
  ...
  // const fetchGateStatistics = useCallback(async (gateId) => {
  //   if (!token || !gateId) return setError("Authentication or gate ID missing.");
  //   setLoading(true);
  //   setError(null);
  //   try {
  //     const data = await fetchGateStats(gateId, token);
  //     setStats(data || null);
  //     return data;
  //   } catch (err) {
  //     return handleAuthError(err);
