...
const listItemStyles = (msg, currentUserId) => ({
  ...
  py: 1,
  backgroundColor:
    !msg.is_read && msg.sender_id !== currentUserId ? "grey.100" : "inherit",
        ...
        {messages.length > 0 ? (
          messages.map((msg) => {
            ...
            return (
              <ListItem
                key={msg.message_id}
                sx={listItemStyles(msg, currentUserId)}
              >
                <ListItemText
                  primary={msg.content}
                  secondary={`From: ${msg.sender_id} | To: ${msg.receiver_id} | ${renderMessageStatus(
                    msg
                  )}`}
                  primaryTypographyProps={{
                    fontWeight:
                      !msg.is_read && !isSentByCurrentUser ? 600 : 400,
                  }}
                />
